...
**先搁置（等美术资源到位、真正要设计全新角色时再做）**：

- **等级曲线**：`character_status` 只有 4 个锚点（1/10/80/100），插值公式未反推。
  这是 Hub 不显示"20 级 HP"的原因——数据支撑不了的数字不显示。
  反推方法已知（改锚点 + 进战斗读数），只是现在做了也没有要落地的数值设计。
- **001 的技能表现**：机制已经是"场地中心六向 30° 展开"，但特效仍是
  `brown_fighter` 的水属性剑舞，与机制不符。换特效要等有可用的美术资源；
  现在换成别的官方特效只是换一个同样不对的皮。

也就是说这两项**不是被卡住，是被排到了它们该在的位置之后**：
先有角色设计和素材，再谈数值曲线和表现。
