...
R3（原创角色 001）需要的依据齐了。参数单位（帧/像素/角度）**三项已全部结案**（见下），还缺
...
### 时间单位是帧、60fps，且各段独立计时（实证）

R2 起就挂着的"参数单位"悬案，第一项已结：**读 bundle 读不出来，但屏幕上看得出来**。

`ShowEffect.lifetime: 347 → 120`，其余不动，`HideCharacter.frame` 留在 347 作对照。实机：

- 特效持续 **约 2 秒** → 120 帧 ÷ 2 秒 = **60fps**（若是 30fps 应为 4 秒）；
  原值 347 帧即约 5.8 秒；
- 特效消失后**角色继续隐身一段时间** → `ShowEffect.lifetime` 与 `HideCharacter.frame`
  是**各自独立的计数器**，不是共用一个"技能总时长"。

后者是能不能分段控制演出的前提：可以按段编排，不必整体缩放。

因此凡是名为 `frame` / `lifetime` / `Wait` 的参数，一律按 **1/60 秒**换算。

### `u` / `v` 是目标局部坐标系里的偏移，直接加到世界坐标上（源码依据）

`ShowEffect.u: 0 → 200` 实机**看不出偏移**。这种时候不该继续猜量级，bundle 里就有答案。

`ShowEffect` 是命令索引 `0x9`，执行处把 `u`/`v` 打包成 `{u,v}` 传给
`ActionEffectManager.show`，最终落在 `ActionEffect.stepPos()`：

```js
this.x = pos.u * cos(dir) - pos.v * sin(dir) + target.getPosX();
this.y = pos.u * sin(dir) + pos.v * cos(dir) + target.getPosY();
```

即：**在目标朝向旋转后的局部坐标系里的平移量，单位与世界坐标同量纲**（逻辑分辨率 540×960）。
没有任何缩放系数——所以 200 是 200 像素，理论上应该看得见。

看不见的合理解释是观测问题而不是单位问题：该特效 `scale=6.0`，画面上本就巨大且大致居中，
而技能期间角色是隐藏的（`HideCharacter`），没有参照物。

但 `u=2000` 那一局仍然读不出结论，原因是判据本身有个没被建立过的前提：
**它默认观察者能把技能特效从画面里认出来**。角色自身的攻击动作同时播放，
画面上没有任何东西区分两者。

所以给特效一个**别的东西都不具备的签名**：`lifetime=600`（10 秒），
而角色攻击动作远不到 1 秒——凡是滞留十秒不散的，按定义就是 `ShowEffect`。
同一局里 `damage` 临时调回官方的 4：秒杀会让战斗一秒结束，
**比要测的东西还短，快速击杀会毁掉它本来要节省的观察窗口**。

实机结果：**没有任何东西滞留十秒**。特效确实被推出了画面之外，
上面那段 `stepPos` 就是实际执行路径。

**结论**：`u`/`v` 是**逻辑像素**（540×960 坐标系），加在目标朝向旋转后的局部坐标系上。
顺带也确认了：此前看到的"技能特效"其实是角色自身的攻击动作。

**方法论（两条，都是这次踩出来的）**：

1. 现象不明显时，先回源码确认参数怎么被消费，再据此设计判据，不要靠加大数值反复试；
2. 让两种结果的**签名不同**（滞留/不滞留），而不是**幅度不同**（偏了多少）。
   幅度判据要求观察者先能认出观察对象——这个前提往往并不成立，
   而它不成立的时候，得到的是一个无法解读的结果，不是一个否定结论。

### `NWay` 分的是**朝向**，不是位置（实证 + 源码）

`formation: ["NWay", 6, 0.5236]` 实机看不出任何"六向展开"。用
`?wfdev=trace:pinball.scene.battle.battle.action.ActionHitArea.stepPos*200~x/y/r`
读出来的判定区状态，一眼就说清了：

```
{"x":414.49,"y":627.07,"r":0.26}    +14.9°
{"x":414.49,"y":627.07,"r":-0.26}   -14.9°
{"x":414.49,"y":627.07,"r":0.79}    +45.3°
{"x":414.49,"y":627.07,"r":-0.79}   -45.3°
{"x":414.49,"y":627.07,"r":1.31}    +75.1°
{"x":414.49,"y":627.07,"r":-1.31}   -75.1°
```

**六块判定区的 `x`/`y` 完全相同，只有 `r` 不同**，而且正是 30° 间隔的
±15° / ±45° / ±75°。源码印证（`ActionHitAreaGroup` 的 formation 分支）：

```js
// NWay：位置偏移写死为 0，只有 w 变
{'u': u + 0*cos(w) - 0*sin(w), 'v': v + 0*sin(w) + 0*cos(w)},
{'w': w + (a/2 + i*a)}
// Line：位置真的偏移
var d = (i*s + 0.5*s); {'u': u + d*cos(w) - d_v*sin(w), ...}
```

**这解释了为什么改了等于没改**：判定形状是 `Circle`，而**圆旋转之后还是同一个圆**。
六块判定区严丝合缝地叠在一起。

两个副产物：

- `ActionHitArea.r` 是**弧度制的朝向**，不是半径（半径在 `shape` 里，是个对象）。
  角度单位由此结案：**弧度**。三项参数单位全部落实。
- 要做空间上的散开，用 `Line(n, s, o)` / `File(n, s, o)`（真的偏移 `u`/`v`），
  `NWay` 只对**有朝向的形状**（`Rectangle` / `Arc`）或会移动的判定有意义。

**方法论**：一个"看不出变化"的改动有两种可能——没生效，或者生效了但在这个配置下
**本来就不该有可见差别**。这次是后者。把内部状态读出来才分得清，
而肉眼判据无论重复多少次都分不清。
